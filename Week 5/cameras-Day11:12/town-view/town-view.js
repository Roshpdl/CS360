...
//red wireframe box around the snowman head
var boxGeom = new THREE.BoxGeometry(2*r1,2*r1,2*r1);
var boxMesh = new THREE.Mesh( boxGeom,
                                new THREE.MeshBasicMaterial({color: THREE.ColorKeywords.red, wireframe: true}));
boxMesh.position.set(smx, 2*r3 + 2*r2 + r1, smz);
scene.add(boxMesh);
...
// *** modify parameters as needed, to create desired view of the town ***
...
// *** replace the following statement with code to create the camera, add it to 
// the scene, and render the scene with the new camera ***
